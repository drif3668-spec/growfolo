...
import {
  BadgeCheck, ChevronDown, ChevronRight, ChevronUp,
...
interface ApiProduct {
  ...
  partners: string[]; features: string[]; specs: Spec[]; faq: FAQ[];
}

function localToApi(p: (typeof PRODUCTS)[number]): ApiProduct {
  return {
    ...
    image_url: null, accent_color: p.accentColor, logo_color: p.color,
    category: p.category, badge: p.badge ?? null,
    rating: p.rating, reviews_count: p.reviews,
    partners: p.partners ?? [], features: p.features, specs: p.specs, faq: [],
  };
}

/* ── Category tabs ─────────────────────────────────────────────── */
const CATEGORIES = [
  { id: "AI",            label: "الذكاء الاصطناعي",  icon: "🤖", active: true,  color: "#a855f7", description: "أدوات AI للإنتاجية والبرمجة والإبداع" },
  { id: "Entertainment", label: "منتجات الترفيه",     icon: "🎬", active: false, color: "#ef4444", description: "نتفليكس، سبوتيفاي، ديزني+ والمزيد" },
  { id: "Content",       label: "صناعة المحتوى",      icon: "🎨", active: false, color: "#f59e0b", description: "أدوبي، كانفا، كابكت وأدوات الإبداع" },
  { id: "Gaming",        label: "شحن الألعاب",         icon: "🎮", active: false, color: "#10b981", description: "PUBG، فري فاير، بلايستيشن وأكثر" },
];

/* ════════════════════════════════════════════════════════════════
   MAIN PAGE — wraps everything in Suspense (required for useSearchParams)
════════════════════════════════════════════════════════════════ */
  ...
  return (
    <Suspense fallback={
      <main className="min-h-screen bg-[#050508]">
        <SiteHeader />
        <div className="mx-auto max-w-6xl px-4 py-20 animate-pulse">
          <div className="h-10 w-56 rounded-2xl bg-white/8 mb-8" />
          <div className="grid gap-5 sm:grid-cols-3">
            {[...Array(6)].map((_, i) => <div key={i} className="h-64 rounded-3xl bg-white/5" />)}
          </div>
        </div>
      </main>
    }>
...
/* ════════════════════════════════════════════════════════════════
   INNER CONTENT — reads ?id= query param
════════════════════════════════════════════════════════════════ */
...
function ProductListing() {
  ...
  useEffect(() => {
    fetch(`${API}/api/v1/products`)
      ...
      .catch(() => setProducts(PRODUCTS.map(localToApi)))
          ...
          <div className="flex gap-1 overflow-x-auto py-3 scrollbar-hide">
            {CATEGORIES.map((cat) => (
              ...
              >
                ...
                <span>{cat.label}</span>
                {!cat.active && (
                  <span className="rounded-md bg-white/8 px-1.5 py-0.5 text-[10px] font-black text-white/30">قريباً</span>
                )}
                {activeCategory === cat.id && (
                  <span className="absolute bottom-0 left-1/2 h-0.5 w-3/4 -translate-x-1/2 rounded-full" style={{ background: catInfo.color }} />
                )}
              </button>
            ))}
          </div>
        ...
        {catInfo.active ? (
          loading ? (
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {[...Array(6)].map((_, i) => <div key={i} className="h-64 animate-pulse rounded-3xl bg-white/5" />)}
            </div>
          ) : filtered.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div
              ...
              style={{ background: `${catInfo.color}15`, border: `1px solid ${catInfo.color}30` }}
            >
              {catInfo.icon}
            </div>
...
function ProductCard({ product }: { product: ApiProduct }) {
  return (
    ...
    >
      ...
      />
      {product.badge && (
        <span className="mb-4 self-start rounded-xl px-3 py-1 text-[11px] font-black text-black" style={{ background: product.accent_color }}>
          {product.badge}
        </span>
      )}
      <div
        ...
        style={{ background: `linear-gradient(135deg, ${product.accent_color}30, ${product.accent_color}10)`, border: `1px solid ${product.accent_color}40`, boxShadow: `0 0 30px ${product.accent_color}20` }}
      >
        {product.logo}
      </div>
      <div className="flex-1">
        ...
        <div className="mt-2 flex items-center gap-1.5">
          ...
          {product.reviews_count > 0 && <span className="text-xs text-white/30">({product.reviews_count.toLocaleString()})</span>}
          {product.buyers && <span className="mr-auto text-xs text-white/30">{product.buyers}+ عميل</span>}
        </div>
      </div>
      <div className="mt-4 flex items-center justify-between">
        <div className="flex items-baseline gap-2">
          <span className="text-2xl font-black text-white">${product.price}</span>
          {product.old_price > 0 && <span className="text-sm text-white/30 line-through">${product.old_price}</span>}
        </div>
        {product.discount && (
          <span className="rounded-lg px-2 py-0.5 text-xs font-black text-black" style={{ background: product.accent_color }}>{product.discount}</span>
        )}
      </div>
    ...
    fetch(`${API}/api/v1/products/${id}`)
      ...
      .then((data: ApiProduct) => {
        ...
        return fetch(`${API}/api/v1/products`)
          .then((r) => r.json())
          .then((all: ApiProduct[]) =>
            setRelated((Array.isArray(all) ? all : []).filter((p) => p.id !== data.id && p.category === data.category).slice(0, 3))
          )
          .catch(() => {
            setRelated(PRODUCTS.filter((p) => p.id !== id && p.category === data.category).slice(0, 3).map(localToApi));
          });
      })
      .catch(() => {
        const local = PRODUCTS.find((p) => p.id === id);
        if (local) {
          setProduct(localToApi(local));
          setRelated(PRODUCTS.filter((p) => p.id !== id && p.category === local.category).slice(0, 3).map(localToApi));
        }
      })
      .finally(() => setLoading(false));
  }, [id]);
    ...
    return (
      <main className="flex min-h-screen flex-col bg-[#050508] text-white">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-4">
          ...
          <a href="/products" className="neon-button rounded-2xl px-6 py-3 font-black text-black">العودة للمنتجات</a>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#050508] text-white" dir="rtl">
      ...
      <section className="relative overflow-hidden">
        <div className="pointer-events-none absolute inset-0" style={{ background: `radial-gradient(ellipse 80% 60% at 50% -20%, ${product.accent_color}18 0%, transparent 70%)` }} />
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#050508]/50 to-[#050508]" />
        <div className="pointer-events-none absolute left-1/4 top-8 size-72 rounded-full blur-[100px] opacity-20" style={{ background: product.accent_color }} />
          ...
          <div className="grid gap-10 md:grid-cols-[auto_1fr]">
            ...
            <div className="relative mx-auto flex shrink-0 flex-col items-center">
              ...
              >
                ...
                <div className="pointer-events-none absolute inset-0 rounded-3xl" style={{ background: "linear-gradient(145deg, rgba(255,255,255,0.12) 0%, transparent 60%)" }} />
              </div>
              <div className="mt-2 h-12 w-40 rounded-b-3xl opacity-20 blur-sm" style={{ background: `linear-gradient(to bottom, ${product.accent_color}50, transparent)` }} />
            </div>

            {/* Info */}
            <div className="flex flex-col justify-center">
              {product.badge && (
                <span className="mb-3 inline-block self-start rounded-xl px-3 py-1 text-xs font-black text-black" style={{ background: product.accent_color }}>
                  {product.badge}
                </span>
              )}
              ...
              <div className="mt-3 flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-1.5">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={14} className={i < Math.round(product.rating) ? "fill-yellow-400 text-yellow-400" : "fill-white/15 text-white/15"} />
                  ))}
                ...
                {product.buyers && (
                  <span className="rounded-xl border border-white/10 bg-white/5 px-3 py-1 text-xs font-bold text-white/50">{product.buyers}+ عميل</span>
                )}
              </div>
              ...
              <div className="mt-6 flex items-end gap-3">
                ...
                {product.old_price > 0 && (
                  <div className="mb-1.5 flex flex-col">
                    ...
                    {product.discount && (
                      <span className="mt-0.5 self-start rounded-lg px-2 py-0.5 text-xs font-black text-black" style={{ background: product.accent_color }}>
        ...
        <div className="grid gap-6 md:grid-cols-[1fr_300px]">
          ...
          <div className="flex flex-col gap-6">
            ...
            {product.full_description && (
              <div className="rounded-3xl border border-white/8 bg-[#0d0b14] p-7">
                ...
                <p className="leading-9 text-white/70 whitespace-pre-line text-[15px]">{product.full_description}</p>
              </div>
            )}
            ...
            {product.benefits && (
              <div className="rounded-3xl p-7" style={{ background: `linear-gradient(135deg, ${product.accent_color}10, ${product.accent_color}05)`, border: `1px solid ${product.accent_color}25` }}>
                  ...
                  {product.faq.map((item, i) => (
                    <div key={i} className="overflow-hidden rounded-2xl border border-white/6 bg-white/3">
                      ...
                      >
                        ...
                        {openFaq === i ? <ChevronUp size={16} className="shrink-0 text-white/40" /> : <ChevronDown size={16} className="shrink-0 text-white/40" />}
                      </button>
                      {openFaq === i && (
                        <div className="border-t border-white/6 px-4 pb-4 pt-3 text-sm leading-7 text-white/60">{item.answer}</div>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <div className="flex flex-col gap-5">
              ...
              <div className="grid gap-3 text-xs text-white/60">
                {[
                  ...
                  <div key={g.text} className="flex items-center gap-2.5">
                    <span className="text-base">{g.icon}</span><span>{g.text}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div
              ...
              style={{ background: `linear-gradient(135deg, ${product.accent_color}18, ${product.accent_color}08)`, border: `1px solid ${product.accent_color}30` }}
            >
              <p className="text-3xl font-black text-white">${product.price}</p>
              {product.old_price > 0 && <p className="mt-0.5 text-sm text-white/35 line-through">${product.old_price}</p>}
              <button
                onClick={handleAddToCart}
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 text-sm text-white/45">
            {[
              ...
              <div key={text} className="flex items-center gap-2">
                <Icon size={15} style={{ color: product.accent_color }} /><span>{text}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="mt-12">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {related.map((rel) => (
                ...
                >
                  <div className="grid size-14 shrink-0 place-items-center rounded-2xl text-2xl font-black transition-transform group-hover:scale-110" style={{ background: `${rel.accent_color}22`, border: `1px solid ${rel.accent_color}33` }}>
                    {rel.logo}
                  </div>
